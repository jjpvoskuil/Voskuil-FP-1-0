...
# Cache key for scored results — persists across reruns (e.g. chat input triggers)
...
if analyze and ticker_input:
    ...
    # Cache results so page stays rendered on chat reruns
...
elif _cache_key and _cache_key in st.session_state and ticker_input:
    # Restore from cache on rerun (chat input, button clicks, etc.)
...
if (analyze and ticker_input) or (_cache_key and _cache_key in st.session_state and ticker_input):
    # Sentinel so the rest of the rendering block always runs when results exist
    _show_results = True
else:
    _show_results = False

if _show_results:
    _dummy = True  # block entry point — all rendering below uses variables set above
...
elif analyze and not ticker_input and not _show_results:
    st.warning("Please enter a ticker symbol to analyze.")
elif not _show_results:
    st.markdown("""
    ### How this works
