"""
sic_codes.py — Complete static SIC (Standard Industrial Classification) code table.

Hand-transcribed from the SEC's official published list:
https://www.sec.gov/search-filings/standard-industrial-classification-sic-code-list
(Last reviewed by SEC: June 18, 2026)

This is a STATIC file, not a live fetch. SIC codes are frozen — last
formally revised in 1987 — and the SEC's own page renders its data table
via JavaScript (Drupal Views), making it unreliable to scrape server-side.
Two scraping attempts (regex, then BeautifulSoup) confirmed this, both
returning empty results despite the page being reachable. A one-time
hand-transcription of the complete table is the most reliable solution.

SIC_FULL: {4-digit code (zero-padded string) -> Title Case industry name}
This is the complete table — every code the SEC publishes.

Used by 8_Market_Screener_EDGAR.py to power the Industry / Sub-Industry
filter dropdowns without any external dependency.
"""
