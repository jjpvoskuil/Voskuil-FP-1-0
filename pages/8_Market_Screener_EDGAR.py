...
_sic_full_count = len(_sic_map.get("full", {}))
if _sic_full_count > 100:
    st.caption(f"✅ SIC sub-industry table loaded ({_sic_full_count} codes from SEC)")
else:
    st.caption(
        f"⚠️ SIC sub-industry table only has {_sic_full_count} codes — the live SEC fetch may "
        f"have failed. Industry filter still works (uses a built-in list); sub-industries may "
        f"be sparse or empty for some/all industries until this is resolved."
    )
